...
class Routine:
    def __init__(self, cfg):
        ...
        if os.path.isfile(linear_modes_file):
            ...
            if os.path.isfile(hess_file):
                ...
                hess_func_single = functorch.hessian(stvk_func_single, argnums=0)
                hess = hess_func_single(self.X)
    ...
    def train(self):
        ...
        while True:
            with torch.no_grad():
                z = torch.rand(batch_size, L, device=self.device) * 1.25 - 0.625
                z[rest_idx,:] = 0
    ...
    def eval(self):
        with torch.no_grad():
            self.model.eval()
            content = torch.load(os.path.join(self.data_dir, 'test_data.pt'), map_location=self.device)
            z = content['z']
            # Forward pass
            L = 3
            linear_modes = self.linear_modes[:,6:6+L]
            y = self.model(z)
            X = self.X.view(1, -1).expand(y.size(0), -1)
            l = X + (z @ linear_modes.T) # [B, 3V]
            stvk = self.stvk_func(y + l, X)
            x = y + l
            # Compute metrics - energy
            self.writer.add_scalar('test/stvk', stvk.mean(), self.cp.iter)
            # Compute metrics - stress
            S_norm_avg, S_norm_max, S_norm_std = self.stvk_func.stressNorm(x, X)
            self.writer.add_scalar('test/stress', S_norm_avg.mean(), self.cp.iter)
...
def main():
    ...
    def GUI_loop():
        ...
        if(psim.TreeNode('Modal coordinate control')):
            for i in range(3):
                changed, engine.state[f'z_{i}'] = psim.SliderFloat(f'Coord {i} (z{i})', engine.state[f'z_{i}'], v_min=-0.5, v_max=0.5)
                update |= changed
            psim.TreePop()
